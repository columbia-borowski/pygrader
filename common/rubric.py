            ...
            for rubric_item in self.depends_on["is_graded"]:
                for i in range(1, len(rubric_item.subitems) + 1):
                    if not grades.is_graded(f"{rubric_item.code}.{i}"):
                        ungraded_dependancies.append(rubric_item)
                        break

            if ungraded_dependancies:
                codes = ", ".join(
                    rubric_item.code for rubric_item in ungraded_dependancies
                )
                p.print_yellow(
...
class Rubric:
    ...
    def _create_rubric_item(self, item_dict: dict) -> RubricItem:
        ...
        return RubricItem(
            ...
            {
                "has_ran": self._create_dependancies_list(
                    depends_on.get("has_ran", [])
                ),
                "is_graded": self._create_dependancies_list(
